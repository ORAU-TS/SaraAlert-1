...
class PublicHealthPatientPageHistoryVerifier < ApplicationSystemTestCase
  ...
  def verify_latest_public_health_action(user_label, latest_public_health_action, reasoning)
    verify_historical_event(user_label, 'Monitoring Change', ['User changed Latest Action', latest_public_health_action, reasoning])
  end

  def verify_additional_public_health_action(user_label)
    verify_historical_event(user_label, 'Monitoring Change', ['User added an additional Health Action'])
  end
