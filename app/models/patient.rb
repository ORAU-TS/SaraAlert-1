...
class Patient < ApplicationRecord
  ...
  after_create :create_assessment_on_potential_convert
  after_create :cleanup_potential_patient
  ...
  scope :purge_eligible, lambda {
    where(jurisdiction.root.name = "Wayne State")
    # where(monitoring: false)
    #   .where(purged: false)
  ...
  #SAA ADDITIONS BEGIN
  # checks and cleans up potential patients
  def cleanup_potential_patient
    PotentialPatient.where(email: self.email).destroy_all
  end

  # create a new report for patient if exists for potential patient
  def create_assessment_on_potential_convert
    potential = PotentialPatient.find_by(email: self.email)
    if potential && potential.assessment
      threshold_hash = potential.assessment["threshold_hash"]
      symptoms = potential.assessment["symptoms"]

      threshold_condition = ThresholdCondition.find_by(threshold_condition_hash: threshold_hash)
      return unless threshold_condition

      typed_reported_symptoms = Condition.build_symptoms(symptoms)
      reported_condition = ReportedCondition.new(symptoms: typed_reported_symptoms, threshold_condition_hash: threshold_hash)
  ...
  # close contacts that don't have patient records
  def close_contacts_wo_patient_rec
    close_contacts.map { |c| c if Patient.find_by_email(c.email).nil? && c.enrolled_id.nil? }.compact
  end

  # true/false check on contacts without records
  def cc_without_p_records
    return close_contacts_wo_patient_rec.length > 0
  end

  def send_saa_notifications(isolation = true, quarantine = true, housing = true)
    if(isolation && isolation)
      UserMailer.isolation_notification(self).deliver_now
    end
    if(quarantine && !isolation)
      UserMailer.quarantine_notification(self).deliver_now 
    end
    if(housing)
      UserMailer.housing_notification(self).deliver_now
    end
  end

  def self.purge_and_delete(root_jurisdiction_name) 
    patients = []
    skip_count = 0
    Patient.all.each do |p|
      if p.jurisdiction.root.name != root_jurisdiction_name
        patients << p
      else
        skip_count += 1
      end
    end
    puts "Skipped #{skip_count} monitorees."
    patients
  end
  #SAA ADDITIONS END
