...
class CloseContact < ApplicationRecord
  ...
  def matching_patient_record
    return Patient.find_by_id(enrolled_id) || Patient.where(email: email).where(first_name: first_name).where(last_name: last_name).first
  end
end
