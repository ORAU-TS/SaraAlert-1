...
class User < ApplicationRecord
  ...
  # Can this user view Patient vaccine results?
  def can_view_patient_vaccines?
    role?(Roles::PUBLIC_HEALTH) || role?(Roles::CONTACT_TRACER) || role?(Roles::PUBLIC_HEALTH_ENROLLER) || role?(Roles::SUPER_USER)
  end

  # Can this user edit Patient vaccine results?
  def can_edit_patient_vaccines?
    role?(Roles::PUBLIC_HEALTH) || role?(Roles::CONTACT_TRACER) || role?(Roles::PUBLIC_HEALTH_ENROLLER) || role?(Roles::SUPER_USER)
  end

  # Can this user create Patient vaccine results?
  ...
  #SAA ADDITIONS BEGIN
  def viewable_potential_patients
    jurisdiction.all_potential_patients
  end

  def get_potential_patient(id)
    if role?(Roles::ENROLLER)
      viewable_potential_patients.find_by_id(id)
    elsif role?(Roles::PUBLIC_HEALTH)
      viewable_potential_patients.find_by_id(id)
    elsif role?(Roles::PUBLIC_HEALTH_ENROLLER) || role?(Roles::SUPER_USER) || role?(Roles::CONTACT_TRACER)
      viewable_potential_patients.find_by_id(id)
