...
class Jurisdiction < ApplicationRecord
  ...
  # All potential patients are those in this or descendant jurisdictions
  def all_potential_patients
    PotentialPatient.includes([:jurisdiction]).where(jurisdiction_id: subtree_ids)
  end
  ...
  ##SAA Additions
  def set_unique_identifier_and_path
    self.unique_identifier = Base64::urlsafe_encode64([[Digest::SHA256.hexdigest(jurisdiction_path_string)].pack('H*')].pack('m0'))[0, 10]

    # Warn user if collision has occured
    if Jurisdiction.where(unique_identifier: unique_identifier).any?
      raise "JURISDICTION IDENTIFIER HASH COLLISION FOR: #{jurisdiction_path_string}"
    end

    self.path = jurisdiction_path_string
  end
end
