...
module ImportExport # rubocop:todo Metrics/ModuleLength
  ...
  EXPORT_TYPES = {
    ...
    csv_linelist_self_enroll: { label: 'Line list CSV (self_enroll)', filename: 'Sara-Alert-Linelist-SelfEnroll'},
    sara_alert_format_self_enroll: { label: 'Sara Alert Format (self_enroll)', filename: 'Sara-Alert-Format-SelfEnroll'}
  }.freeze
  ...
  LINELIST_FIELDS = %i[id name jurisdiction_name assigned_user user_defined_id_statelocal sex date_of_birth end_of_monitoring exposure_risk_assessment
                       ...
                       transferred_to expected_purge_ts symptom_onset extended_isolation saa_student_id saa_building_name saa_floor_number saa_room_number
                       supervisor department project].freeze

  LINELIST_POTENTIAL_FIELDS = %i[first_name last_name email].freeze

  LINELIST_POTENTIAL_HEADERS = ['First Name', 'Last Name', 'Email'].freeze
  ...
  LINELIST_HEADERS = ['Patient ID', 'Monitoree', 'Jurisdiction', 'Assigned User', 'State/Local ID', 'Sex', 'Date of Birth', 'End of Monitoring', 'Risk Level',
                      ...
                      'Transferred From', 'Transferred To', 'Expected Purge Date', 'Symptom Onset', 'Extended Isolation',
                      'Monitoree ID', 'Building Name', 'Floor Number', 'Room Number', 'Supervisor', 'Department', 'Project'].freeze
  ...
  SARA_ALERT_FORMAT_FIELDS = %i[first_name middle_name last_name date_of_birth sex white black_or_african_american american_indian_or_alaska_native asian
                                ...
                                user_defined_id_statelocal user_defined_id_cdc user_defined_id_nndss supervisor department project
                                address_line_1 address_city address_state address_line_2
                                ...
                                case_status
                                lab_1_type lab_1_specimen_collection lab_1_report lab_1_result lab_1_location
                                lab_2_type lab_2_specimen_collection lab_2_report lab_2_result lab_2_location
                                jurisdiction_path assigned_user gender_identity sexual_orientation
                                saa_student_id saa_building_name saa_floor_number saa_room_number
                                vac_1_vaccinated vac_1_disease vac_1_vaccine_type vac_1_lot_number vac_1_first_vac_date vac_1_second_vac_date
                                vac_2_vaccinated vac_2_disease vac_2_vaccine_type vac_2_lot_number vac_2_first_vac_date vac_2_second_vac_date
                              ].freeze
  ...
  SARA_ALERT_FORMAT_HEADERS = ['First Name', 'Middle Name', 'Last Name', 'Date of Birth', 'Sex at Birth', 'White', 'Black or African American',
                               ...
                               'Identifier (NNDSS)', 'Supervisor', 'Department', 'Project', 
                               'Address Line 1', 'Address City', 'Address State', 'Address Line 2', 'Address Zip', 'Address County',
                               ...
                               'Exposure Notes', 'Status', 'Symptom Onset Date', 'Case Status',
                               'Lab 1 Test Type', 'Lab 1 Specimen Collection Date', 'Lab 1 Report Date', 'Lab 1 Result', 'Lab 1 Location',
                               'Lab 2 Test Type', 'Lab 2 Specimen Collection Date', 'Lab 2 Report Date', 'Lab 2 Result', 'Lab 2 Location',
                               'Full Assigned Jurisdiction Path', 'Assigned User', 'Gender Identity', 'Sexual Orientation',
                               'Monitoree ID', 'Building Name', 'Floor Number', 'Room Number', 
                               'Vac 1 Vaccinated', 'Vac 1 Disease', 'Vac 1 Vaccine Type', 'Vac 1 Lot Number', 'Vac 1 First Vac Date', 'Vac 1 Second Vac Date',
                               'Vac 2 Vaccinated', 'Vac 2 Disease', 'Vac 2 Vaccine Type', 'Vac 2 Lot Number', 'Vac 2 First Vac Date', 'Vac 2 Second Vac Date',
                              ].freeze

  SARA_ALERT_POTENTIAL_FORMAT_FIELDS = %i[first_name last_name email].freeze

  SARA_ALERT_POTENTIAL_FORMAT_HEADERS = ['First Name', 'Last Name', 'Email'].freeze
  ...
  FULL_HISTORY_LABORATORIES_FIELDS = %i[patient_id lab_type  specimen_collection report result location created_at updated_at].freeze

  FULL_HISTORY_LABORATORIES_HEADERS = ['Patient ID', 'Lab Type', 'Specimen Collection Date', 'Report Date', 'Result', 'Lab Location', 'Created At', 'Updated At'].freeze
  ...
  PATIENT_LAB_FIELDS = %i[lab_1_type lab_1_specimen_collection lab_1_report lab_1_result lab_1_location lab_2_type lab_2_specimen_collection lab_2_report lab_2_result lab_2_location].freeze

  # fields to include: vac_1_vaccinated, vac_1_disease, vac_1_vaccine_type, vac_1_lot_number, vac_1_first_vac_date, vac_1_second_vac_date
  PATIENT_VACCINE_FIELDS = %i[vac_1_vaccinated vac_1_disease vac_1_vaccine_type vac_1_lot_number vac_1_first_vac_date vac_1_second_vac_date 
                              vac_2_vaccinated vac_2_disease vac_2_vaccine_type vac_2_lot_number vac_2_first_vac_date vac_2_second_vac_date].freeze
  ...
  PATIENT_FIELD_TYPES = {
    ...
    strings: %i[first_name middle_name last_name sex ethnicity primary_language secondary_language nationality user_defined_id_statelocal user_defined_id_cdc
                user_defined_id_nndss supervisor department project
                address_line_1 address_city address_state address_line_2 address_zip address_county foreign_address_line_1
  ...
  VACCINE_FIELD_NAMES = {
    patient_id: 'Sara Alert ID',
    id: 'Vaccine ID',
    created_at: 'Vaccine Created Date',
    updated_at: 'Vaccine Updated Date',
    disease: 'Disease',
    first_vac_date: 'First Vaccine Date',
    second_vac_date: 'Second Vaccine Date',
    lot_number: 'Lot Number',
    vac_type: 'Vaccine Type',
    vaccinated: 'Vaccinated',
  }.freeze

  POTENTIAL_PATIENT_FIELD_NAMES = {
    first_name: 'First Name',
    last_name: 'Last Name',
    email: 'Email',
  }
  ...
  ALL_FIELDS_NAMES = {
    ...
    potential_patients: POTENTIAL_PATIENT_FIELD_NAMES,
  }.freeze
  ...
  LABORATORIES_EXPORT_OPTIONS = {
    ...
    nodes: [rct_node(:laboratories, 'Lab Results', %i[patient_id user_defined_id_statelocal user_defined_id_cdc user_defined_id_nndss id lab_type
                                                      specimen_collection report result location created_at updated_at])]
  }.freeze
  ...
  VACCINE_EXPORT_OPTIONS = {
    label: 'Vaccines',
    nodes: [rct_node(:vaccines, 'Vaccines', %i[patient_id vaccinated first_vac_date second_vac_date created_at updated_at disease vac_type lot_number])]
  }.freeze

  POTENTIAL_PATIENT_EXPORT_OPTIONS = {
    label: 'Potential Patients',
    nodes: [rct_node(:potential_patients, 'Potential Patients', %i[first_name last_name email])]
  }.freeze
  ...
  CUSTOM_EXPORT_OPTIONS = {
    ...
    vaccines: VACCINE_EXPORT_OPTIONS,
  }.freeze
  ...
  # retrieves potential patient data in a standardized way
  def get_potential_patient_data(patients, data)
    exported_data = []

    patients.each do |patient|
      details = {}
      details[:first_name] = patient.first_name
      details[:last_name] = patient.last_name
  ...
  def extract_patients_details(patients, fields)
    ...
    patients_vaccines = get_patients_vaccines(patients) if (fields & PATIENT_VACCINE_FIELDS).any?
    ...
    patients.each do |patient|
      ...
      if patients_laboratories&.key?(patient.id)
        if patients_laboratories[patient.id]&.first&.present?
          ...
          patient_details[:lab_1_location] = patients_laboratories[patient.id].first[:location] || '' if fields.include?(:lab_1_location)
        end
        if patients_laboratories[patient.id]&.second&.present?
          ...
          patient_details[:lab_2_location] = patients_laboratories[patient.id].first[:location] || '' if fields.include?(:lab_2_location)
        end
      end
      ...
      if patients_vaccines&.key?(patient.id)
        if patients_vaccines[patient.id]&.first&.present?
          patient_details[:vac_1_vaccinated] = patients_vaccines[patient.id].first[:vaccinated] || '' if fields.include?(:vac_1_vaccinated)
          patient_details[:vac_1_disease] = patients_vaccines[patient.id].first[:disease] || '' if fields.include?(:vac_1_disease)
          patient_details[:vac_1_vaccine_type] = patients_vaccines[patient.id].first[:vac_type] || '' if fields.include?(:vac_1_vaccine_type)
          patient_details[:vac_1_lot_number] = patients_vaccines[patient.id].first[:lot_number] || '' if fields.include?(:vac_1_lot_number)
          patient_details[:vac_1_first_vac_date] = patients_vaccines[patient.id].first[:first_vac_date]&.strftime('%F') || '' if fields.include?(:vac_1_first_vac_date)
          patient_details[:vac_1_second_vac_date] = patients_vaccines[patient.id].first[:second_vac_date]&.strftime('%F') || '' if fields.include?(:vac_1_second_vac_date)
        end
        if patients_vaccines[patient.id]&.second&.present?
          patient_details[:vac_2_vaccinated] = patients_vaccines[patient.id].second[:vaccinated] || '' if fields.include?(:vac_2_vaccinated)
          patient_details[:vac_2_disease] = patients_vaccines[patient.id].second[:disease] || '' if fields.include?(:vac_2_disease)
          patient_details[:vac_2_vaccine_type] = patients_vaccines[patient.id].second[:vac_type] || '' if fields.include?(:vac_2_vaccine_type)
          patient_details[:vac_2_lot_number] = patients_vaccines[patient.id].second[:lot_number] || '' if fields.include?(:vac_2_lot_number)
          patient_details[:vac_2_first_vac_date] = patients_vaccines[patient.id].second[:first_vac_date]&.strftime('%F') || '' if fields.include?(:vac_2_first_vac_date)
          patient_details[:vac_2_second_vac_date] = patients_vaccines[patient.id].second[:second_vac_date]&.strftime('%F') || '' if fields.include?(:vac_2_second_vac_date)
        end
      end
  ...
  def extract_incomplete_patient_details(patient, fields)
    ...
    #SAA fields
    patient_details[:saa_student_id] = patient.saa_student_id || '' if fields.include?(:saa_student_id)
    patient_details[:saa_building_name] = patient.saa_building_name || '' if fields.include?(:saa_building_name)
    patient_details[:saa_floor_number] = patient.saa_floor_number || '' if fields.include?(:saa_floor_number)
    patient_details[:saa_room_number] = patient.saa_room_number || '' if fields.include?(:saa_room_number)
  ...
  # Gets a hash of the 2 latest vaccines of each patient
  def get_patients_vaccines(patients)
    Vaccine.where(patient_id: patients.pluck(:id)).order(updated_at: :desc).group_by(&:patient_id).transform_values { |v| v.take(2) }
  end
  ...
  # Writes export data to file(s)
  def write_export_potential_data_to_files(config, patients_group, index, inner_batch_size)
    case config[:format]
    when 'csv'
      csv_potential_patient_export(config, patients_group, index, inner_batch_size)
    when 'xlsx'
      xlsx_potential_patient_export(config, patients_group, index, inner_batch_size)
    end
  end
  ...
  def csv_potential_patient_export(config, patients_group, index, inner_batch_size)
    # build package that holds the potential patient data
    package = CSV.generate(headers: true) do |csv|
      csv << LINELIST_POTENTIAL_HEADERS

      # build in batches to reduce memory footprint
      patients_group.in_batches(of: inner_batch_size) do |batch_group|
        exported_data = get_potential_patient_data(batch_group.order(:id), config[:data])
  
        exported_data&.each do |record|
          csv << ["#{record[:first_name]}","#{record[:last_name]}","#{record[:email]}"]
        end
      end
    end

    # build packages to use format from patient export, build file
    packages = {:patients => package}
    data_type = :self_enroll
    file = { filename: build_export_filename(config, data_type, index, false), content: Base64.encode64(packages[:patients]) }
  end
  ...
  def xlsx_potential_patient_export(config, patients_group, index, inner_batch_size)
    # build package that holds the potential patient data
    data_type = :self_enroll
    Axlsx::Package.new do |package|
      package.workbook.add_worksheet(name: 'Self Enrolled') do |sheet|
        sheet.add_row(LINELIST_POTENTIAL_HEADERS)
        ...
        patients_group.in_batches(of: inner_batch_size) do |batch_group|
          ...
          exported_data&.each do |record|
            sheet.add_row(["#{record[:first_name]}","#{record[:last_name]}","#{record[:email]}"])
          end
        end
      end

      return { filename: build_export_filename(config, data_type, index, false), content: Base64.encode64(package.to_stream.read) }
    end
  end
