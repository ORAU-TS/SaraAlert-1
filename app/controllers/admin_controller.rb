...
class AdminController < ApplicationController
  ...
  def users
    ...
    users.each do |user|
      details = {
        ...
        role_title: user.role.titleize.saa_role_name,
  ...
  def create_user
    ...
    role = role.sa_role_name.split(' ').map(&:downcase).join('_')
    ...
    render(json: {err: user.errors.full_messages}, status: 422) && return unless user.save
