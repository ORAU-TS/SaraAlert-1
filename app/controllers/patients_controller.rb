...
class PatientsController < ApplicationController
  ...
  def new
    ...
    supervisors = Patient.where(jurisdiction_id: current_user.jurisdiction_id).distinct.order(:supervisor).pluck(:supervisor).reject(&:blank?)
    departments = Patient.where(jurisdiction_id: current_user.jurisdiction_id).distinct.order(:department).pluck(:department).reject(&:blank?)
    projects = Patient.where(jurisdiction_id: current_user.jurisdiction_id).distinct.order(:project).pluck(:project).reject(&:blank?)
    @special_id_fields = {
  ...
  # Similar to 'new', expect converting a potential patient to a patient
  def convert_potential_patient
    redirect_to(root_url) && return unless current_user.can_create_patient?

    potential_patient = current_user.get_potential_patient(params.permit(:id)[:id])

    redirect_to(root_url) && return if potential_patient.nil?

    @patient = Patient.new(jurisdiction_id: potential_patient.jurisdiction_id,
                           isolation: false,
                           first_name: potential_patient.first_name,
                           last_name: potential_patient.last_name,
                           email: potential_patient.email)
  end
  ...
  def edit
    ...
    supervisors = Patient.where(jurisdiction_id: @patient.jurisdiction_id).distinct.order(:supervisor).pluck(:supervisor).reject(&:blank?)
    departments = Patient.where(jurisdiction_id: @patient.jurisdiction_id).distinct.order(:department).pluck(:department).reject(&:blank?)
    projects = Patient.where(jurisdiction_id: @patient.jurisdiction_id).distinct.order(:project).pluck(:project).reject(&:blank?)
    @special_id_fields = {
      supervisor: supervisors,
      department: departments,
      project: projects,
