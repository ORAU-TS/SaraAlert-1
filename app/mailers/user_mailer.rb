...
class UserMailer < ApplicationMailer
  ...
  def assessment_job_email(sent, not_sent, eligible)
    ...
    @not_sent = not_sent
    @eligible = eligible
    mail(to: ADMIN_OPTIONS['job_run_email'], subject: "Sara Alert Academic Send Assessments Job Results (#{ActionMailer::Base.default_url_options[:host]})")
  end
  ...
  def purge_job_email(purged, not_purged, eligible)
    ...
    @eligible = eligible
    mail(to: ADMIN_OPTIONS['job_run_email'], subject: "Sara Alert Academic Purge Job Results (#{ActionMailer::Base.default_url_options[:host]})")
  end
  ...
  def close_job_email(closed, not_closed, eligible)
    ...
    @eligible = eligible
    mail(to: ADMIN_OPTIONS['job_run_email'], subject: "Sara Alert Academic Close Job Results (#{ActionMailer::Base.default_url_options[:host]})")
  end
  ...
  def cache_analytics_job_email(cached, not_cached, eligible)
    ...
    @eligible = eligible
    mail(to: ADMIN_OPTIONS['job_run_email'], subject: "Sara Alert Academic Cache Analytics Job Results (#{ActionMailer::Base.default_url_options[:host]})")
  end
  ...
  def download_email(user, export_label, lookups, batch_size)
    ...
    mail(to: user.email.strip, subject: 'Your Sara Alert Academic system export is ready') do |format|
      format.html { render layout: 'main_mailer' }
    end
  end
  ...
  def welcome_email(user, password)
    @user = user
    @password = password
    mail(to: user.email.strip, subject: 'Welcome to the Sara Alert Academic system') do |format|
      format.html { render layout: 'main_mailer' }
    end
  end
  ...
  def admin_message_email(user, comment)
    @comment = comment
    mail(to: user.email.strip, subject: 'Message from the Sara Alert Academic system') do |format|
      format.html { render layout: 'main_mailer' }
    end
  end
  ...
  def purge_notification(user, num_purgeable_records)
    ...
    subject = @num_purgeable_records.zero? ? 'Sara Alert Academic Notification' : 'Sara Alert Academic User Records Expiring Soon'
  ...
  def isolation_notification(patient)
    custom_message = CustomMessage.isolation_email_message(patient)

    @subject = custom_message[:subject]
    @body = custom_message[:body]
    mail(to: custom_message[:recipients], subject: @subject) do |format|
      format.html { render layout: 'main_mailer' }
    end
  end

  def quarantine_notification(patient)
    custom_message = CustomMessage.quarantine_email_message(patient)
  ...
  def housing_notification(patient)
    custom_message = CustomMessage.housing_email_message(patient)
