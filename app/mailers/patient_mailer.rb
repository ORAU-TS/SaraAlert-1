...
class PatientMailer < ApplicationMailer
  ...
  def enrollment_email(patient)
    ...
    # SAA - During enrollment, you are always the only patient even when clicking "Finish and add household member"
    # This behavior is in line with sms_weblink and sms_text_based
    @patient = { patient: patient, jurisdiction_unique_id: Jurisdiction.find_by_id(patient.jurisdiction_id).unique_identifier }
    hoh_message = CustomMessage.enrollment_email_message(patient)
    @email_body = hoh_message[:body]
    customize_email_shared(patient)

    mail(to: patient.email&.strip, subject: hoh_message[:subject]) do |format|
  ...
  def enrollment_sms_text_based(patient)
    ...
    contents = CustomMessage.enrollment_sms_text_based_message(patient)
  ...
  def assessment_sms_weblink(patient)
    ...
    patient.active_dependents.uniq.each do |dependent|
      ...
      contents = CustomMessage.assessment_sms_weblink_message(dependent, url)
  ...
  def assessment_email(patient)
    ...
    @patients = patient.active_dependents.uniq.collect do |dependent| {
        ...
        email_body: CustomMessage.assessment_email_message(patient)[:body]
      }
    end
    hoh_message = CustomMessage.assessment_email_message(patient)
    patient.update(last_assessment_reminder_sent: DateTime.now) # Update last send attempt timestamp before SMTP call
    customize_email_shared(patient)
    mail(to: patient.email&.strip, subject: hoh_message[:subject], locale: @lang || :en) do |format|
      format.html { render layout: 'main_mailer' }
    end
    add_success_history(patient, patient)
  ...
  def closed_email(patient)
    ...
    @patient = patient
    @lang = patient.select_language
    hoh_message = CustomMessage.monitoring_complete_email_message(patient)
    @email_body = hoh_message[:body]
    customize_email_shared(patient)
    mail(to: patient.email&.strip, subject: hoh_message[:subject]) do |format|
      format.html { render layout: 'main_mailer' }
  ...
  def customize_email_shared(patient)
    @report_button_text = CustomMessage.shared_button_text_message(patient)
    @shared_footer = CustomMessage.shared_footer_text_message(patient)
  end
end
