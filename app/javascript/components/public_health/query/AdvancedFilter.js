  ...
  constructor(props) {
    ...
    this.state = {
      ...
      filterOptions: [
        ...
        {
          name: 'monitoree-id',
          title: 'Monitoree Id (Number)',
          description: 'Find a specific monitoree record for the given Id',
          type: 'search',
        },
        {
          name: 'building-name',
          title: 'Building Name (Text)',
          description: 'Find all monitorees in a given building',
          type: 'search',
        },
        {
          name: 'floor-number',
          title: 'Floor (Text)',
          description: 'Find all monitorees on a given floor',
          type: 'search',
        },
        {
          name: 'room-number',
          title: 'Room (Text)',
          description: 'Find all monitorees in a given room',
          type: 'search',
  ...
  componentDidMount() {
    ...
    axios.get(window.BASE_PATH + '/user_filters' + timestamp).then(response => {
      this.setState({ savedFilters: response.data }, () => {
        ...
        let sessionFilter = localStorage.getItem(`${window.USER_ID}SaraFilter`);
  ...
  clear = () => {
    this.setState({ activeFilterOptions: [], show: false, activeFilter: null, applied: false }, () => {
      ...
      if (this.props.updateStickySettings) {
        localStorage.setItem(`${window.USER_ID}SaraFilter`, null);
      }
    });
  };
  ...
  setFilter = (filter, apply = false) => {
    if (filter) {
      this.setState({ activeFilter: filter, show: true, activeFilterOptions: filter?.contents || [] }, () => {
        if (this.props.updateStickySettings) {
          localStorage.setItem(`${window.USER_ID}SaraFilter`, filter.id);
        }
        if (apply) {
          this.apply();
        }
