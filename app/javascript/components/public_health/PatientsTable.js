...
import BadgeCC from '../util/BadgeCC';
...
class PatientsTable extends React.Component {
    ...
    this.state = {
      table: {
        colData: [
          ...
          { field: 'public_health_action', label: 'Latest Student Health Action', isSortable: true, tooltip: null },
          ...
          { field: 'latest_comment', label: 'Latest Comment', isSortable: false, tooltip: null, filter: this.linkComment },
  ...
  componentDidMount() {
    ...
    let tab = localStorage.getItem(`${window.USER_ID}${this.props.workflow}Tab`);
    ...
    let jurisdiction = localStorage.getItem(`${window.USER_ID}SaraJurisdiction`);
    if (jurisdiction) {
      this.handleJurisdictionChange(parseInt(jurisdiction));
    }

    // Set assigned user if it exists in local storage
    let assigned_user = localStorage.getItem(`${window.USER_ID}SaraAssignedUser`);
    if (assigned_user) {
  ...
  clearAllFilters = async () => {
    if (await confirmDialog('Are you sure you want to clear all filters? All active filters and searches will be cleared.')) {
      localStorage.removeItem(`${window.USER_ID}SaraFilter`);
      localStorage.removeItem(`${window.USER_ID}SaraPage`);
      localStorage.removeItem(`${window.USER_ID}SaraEntries`);
      localStorage.removeItem(`${window.USER_ID}SaraSearch`);
      localStorage.removeItem(`${window.USER_ID}SaraJurisdiction`);
      localStorage.removeItem(`${window.USER_ID}SaraAssignedUser`);
      localStorage.removeItem(`${window.USER_ID}SaraScope`);
      location.reload();
      this.setState(state => {
  ...
  handleJurisdictionChange = jurisdiction => {
    if (jurisdiction !== this.state.query.jurisdiction) {
      ...
      localStorage.removeItem(`${window.USER_ID}SaraPage`);
      localStorage.setItem(`${window.USER_ID}SaraJurisdiction`, jurisdiction);
    }
  };
  ...
  handleScopeChange = scope => {
    if (scope !== this.state.query.scope) {
      ...
      localStorage.removeItem(`${window.USER_ID}SaraPage`);
      localStorage.setItem(`${window.USER_ID}SaraScope`, scope);
    }
  };
  ...
  handleAssignedUserChange = user => {
    if (user !== this.state.query.user) {
      ...
      localStorage.removeItem(`${window.USER_ID}SaraPage`);
      if (user) {
        localStorage.setItem(`${window.USER_ID}SaraAssignedUser`, user);
      } else {
        localStorage.removeItem(`${window.USER_ID}SaraAssignedUser`);
      }
    }
  ...
  linkPatient = data => {
    ...
    return (
      <div>
        {rowData.is_hoh && <BadgeHOH patientId={rowData.id.toString()} customClass={'badge-hoh ml-1'} location={'right'} />}
        {rowData.has_cc_wo_patient && <BadgeCC patientId={rowData.id.toString()} customClass={'badge-cc ml-1'} location={'right'} />}
  ...
  linkComment = data => {
    const comment = data.rowData.latest_comment || '';
    const comment_id = data.rowData.comment_id || '';
    const id = data.rowData.id;
    const trim_comment = comment.length > 30 ? comment.substring(0, 30) + '...' : comment;
    return <a href={`/patients/${id}#comment_${comment_id}`}>{trim_comment}</a>;
  };
