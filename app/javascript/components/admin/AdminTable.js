...
import { Button, Card, DropdownButton, Dropdown, InputGroup, Form, OverlayTrigger, Tooltip } from 'react-bootstrap';
...
class AdminTable extends React.Component {
  ...
  addUser = data => {
    ...
    const errorMsg = err => (
      <div>
        User not created. Please review the following errors:
        {err && err.map((e, i) => <div key={i}>{e}</div>)}
      </div>
    );

    const handleError = error => {
      const error_list = error.response.data.err;
      toast.error(errorMsg(error_list), {
        autoClose: 2000,
        position: toast.POSITION.TOP_CENTER,
      });
    };
  ...
  render() {
    return (
      <div className="mx-2">
        ...
        <Card className="mt-3 mb-4 card-square">
