...
const TOOLTIP_TEXT = {
  ...
  monitoringStatus: (
    <div>
      ...
      , the system moves the record to the appropriate line list based on reporting history and latest actions. The system will send daily report reminders if
      the record is not on the <i>Records Requiring Review</i> (isolation) line list. If set to{' '}
      <i>
        <b>Not Monitoring</b>
      </i>
  ...
  latestPublicHealthActionInIsolation: (
    <div>
      Used to note the health recommendation provided to a monitoree. In the isolation workflow, this element does not impact the line list on which this record
      appears.
    </div>
  ),
  ...
  vacRecords: <div>Vaccine Records are used to track vaccine history for enrolled monitorees.</div>,
  ...
  exposure_symptomatic: (
    <div>
      Monitorees on this list require health services follow-up to determine if disease is suspected. Follow-up should be based on current guidelines and
      available resources.
    </div>
  ),
  ...
  exposure_non_reporting: (
    <div>
      Monitorees on this list require health services follow-up to collect missing symptom report(s). Follow-up with these monitorees should be based on current
      guidelines and available resources.
    </div>
  ),

  exposure_asymptomatic: <div>Monitorees on this list do not require health services follow-up unless otherwise indicated.</div>,

  exposure_under_investigation: (
    <div>
      A ‘Latest Action’ other than “None” has been documented in the monitoree’s record. Monitorees on this list do not receive daily reminder notifications
      because they are already being contacted by health services.
    </div>
  ),
  ...
  isolation_records_requiring_review: (
    <div>
      These cases meet one of the recovery definitions and require review by health services to validate that it is safe to discontinue isolation. The recovery
  ...
  isolation_non_reporting: (
    <div>
      Monitorees on this list require health services follow-up to collect missing symptom report(s). Follow-up with these cases should be based on current
      guidelines and available resources.
    </div>
  ),

  isolation_reporting: <div>Monitorees on this list do not require health services follow-up unless otherwise indicated.</div>,
  ...
  // SELF-ENROLL WORKFLOW LINE LIST DEFINITIONS
  self_enrolled_patients: (
    <div>
      Monitorees on this list have self enrolled and require follow-up to determine if disease is suspected. Follow-up should be based on current guidelines and
      available resources.
    </div>
  ),
