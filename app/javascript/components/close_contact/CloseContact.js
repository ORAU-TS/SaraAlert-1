...
class CloseContact extends React.Component {
  ...
  handleViewRecord = () => {
    if (this.props.close_contact.enrolled_id) {
      location.href = window.BASE_PATH + '/patients/' + this.props.close_contact.enrolled_id;
    } else {
      location.href = window.BASE_PATH + '/patients/' + this.props.cc_patient.id;
    }
  };
  ...
  render() {
    return (
      <React.Fragment>
        ...
        {this.props.close_contact.id && (this.props.close_contact.enrolled_id || this.props.cc_patient) && (
          <div className="pl-2">
            <Button variant="link" onClick={this.handleViewRecord} className="btn btn-link py-0" size="sm">
              <i className="fas fa-search"></i> View Record
            </Button>
          </div>
        )}
        {this.props.close_contact.id && !this.props.cc_patient && !this.props.close_contact.enrolled_id && this.props.can_enroll_patient_close_contacts && (
          <div className="pl-2">
            <Button
              variant="link"
              onClick={() => {
