...
import { floorOptions } from '../../../data/floorOptions';
...
class Address extends React.Component {
  ...
  validate = callback => {
    ...
    Promise.all([
      schemaBoth.validate(this.state.current.patient, { abortEarly: false }),
      this.state.selectedTab === 'domestic'
        ? schemaDomestic.validate(this.state.current.patient, { abortEarly: false })
        : schemaForeign.validate(this.state.current.patient, { abortEarly: false }),
    ])
      .then(function() {
    ...
    return (
      <React.Fragment>
        ...
        <Card className="mx-2 card-square">
          ...
          <Card.Body>
            <Form.Group controlId="saa_building_name">
              <Form.Label className="nav-input-label">BUILDING NAME{schemaBoth?.fields?.saa_building_name?._exclusive?.required && ' *'}</Form.Label>
              <Form.Control
                isInvalid={this.state.errors['saa_building_name']}
                as="select"
                size="lg"
                className="form-square"
                value={this.state.current.patient.saa_building_name || ''}
                onChange={this.handleChange}>
                <option></option>
                {this.props.buildings.map((building, index) => (
                  <option key={`floor-${index}`}>{building}</option>
                ))}
              </Form.Control>
              <Form.Control.Feedback className="d-block" type="invalid">
                {this.state.errors['saa_building_name']}
              </Form.Control.Feedback>
            </Form.Group>
            <Form.Row>
              <Form.Group as={Col} controlId="saa_floor_number">
                <Form.Label className="nav-input-label">FLOOR NUMBER{schemaBoth?.fields?.saa_floor_number?._exclusive?.required && ' *'}</Form.Label>
                <Form.Control
                  isInvalid={this.state.errors['saa_floor_number']}
                  as="select"
                  size="lg"
                  className="form-square"
                  value={this.state.current.patient.saa_floor_number || ''}
                  onChange={this.handleChange}>
                  <option></option>
                  {floorOptions.map((floor, index) => (
                    <option key={`floor-${index}`}>{floor}</option>
                  ))}
                </Form.Control>
                <Form.Control.Feedback className="d-block" type="invalid">
                  {this.state.errors['saa_floor_number']}
                </Form.Control.Feedback>
              </Form.Group>
              <Form.Group as={Col} controlId="saa_room_number">
                <Form.Label className="nav-input-label">ROOM NUMBER{schemaBoth?.fields?.saa_room_number?._exclusive?.required && ' *'}</Form.Label>
                <Form.Control
                  isInvalid={this.state.errors['saa_room_number']}
                  size="lg"
                  className="form-square"
                  value={this.state.current.patient.saa_room_number || ''}
                  onChange={this.handleChange}
                />
                <Form.Control.Feedback className="d-block" type="invalid">
                  {this.state.errors['saa_room_number']}
                </Form.Control.Feedback>
              </Form.Group>
            </Form.Row>
            <Tabs
              ...
              <Tab eventKey="foreign" title="Home Address Outside USA (Foreign)">
                <Form>
                  ...
                  <Form.Row className="h-100">
                    <Form.Group as={Col} md={24} className="my-auto">
                      <div className="h5">Address at Destination in USA Where Monitored</div>
                    </Form.Group>
                  </Form.Row>
                  <Form.Row className="pb-2">
                    <Form.Group as={Col} md={24} className="my-auto">
...
const schemaBoth = yup.object().shape({
  saa_building_name: yup
    .string()
    .max(200, 'Max length exceeded, please limit to 200 characters.')
    .nullable(),
  saa_floor_number: yup
    .string()
    .max(200, 'Max length exceeded, please limit to 200 characters.')
    .nullable(),
  saa_room_number: yup.string().nullable(),
});
