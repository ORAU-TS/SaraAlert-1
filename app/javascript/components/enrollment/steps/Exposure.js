...
class Exposure extends React.Component {
  ...
  handleHasOverlap = () => {
    const patient = this.state.modified.patient ? { ...this.state.current.patient, ...this.state.modified.patient } : { ...this.state.current.patient };
    const hasOverlap = HasOverlap(patient);
    this.setState({
      saa_has_overlap: hasOverlap,
    });
  };
  ...
  handleDateChange(field, date) {
    ...
    if (field === 'saa_last_day_at_work') {
      if (modified.patient) {
        modified.patient.saa_last_day_at_work = date;
      } else {
        modified = { patient: { saa_last_day_at_work: date } };
      }
    }
                ...
                <Form.Group as={Col} className="my-auto">
                  ...
                  <Form.Row className="pt-2">
                    <Form.Group as={Col} className="mb-2">
                      <Form.Label className="nav-input-label">RISK ASSESSMENT AND MANAGEMENT</Form.Label>
                    </Form.Group>
                  </Form.Row>
                  <Form.Row>
