...
class Enrollment extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      enrollmentState: {
        ...
        special_id_fields: this.props.special_id_fields,
      },
    };
      ...
      .then(response => {
        if (response.data && response.data.is_duplicate) {
          ...
          if (this.props.editMode) {
            toast.success(message, { onClose: () => (location.href = redirectUrl) });
          } else {
  ...
  handleScroll = () => {
    let self = this;
    setTimeout(() => {
      self.scrollTarget.current.scrollIntoView({ behavior: 'instant' });
    }, 400);
  };
  ...
  render() {
    return (
      <React.Fragment>
        <div>
          <a ref={this.scrollTarget} className="anchor"></a>
          <Carousel
            controls={false}
            indicators={false}
            interval={null}
...
Enrollment.propTypes = {
  ...
  special_id_fields: PropTypes.object,
};
